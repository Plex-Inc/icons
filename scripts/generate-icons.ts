...
function transformAttr(key: string, value: string, escape: (str: string) => string) {
...
async function generateIcon(iconName: string, filePath: string): Promise<string> {
    ...
    for (const type of iconTypes) {
        ...
        for (const file of files) {
            if (path.extname(file) === '.svg') {
                const iconName = `${toPascalCase(path.basename(file, '.svg'))}${toPascalCase(type)}`;
                const componentContent = await generateIcon(iconName, path.join(srcDir, file));
